...
export class PostRaceScene implements Scene {
  ...
  private summary = new Text('Race complete! The cortical reaction prevents polyspermy. Great job exploring the journey!', {
    fill: 0xffffff,
    fontSize: 22,
    wordWrap: true,
    wordWrapWidth: 640,
  });
  private button = new Text('Return to Menu', { fill: 0x8ae6ff, fontSize: 26 });

  constructor(private manager: SceneManager) {
    const bg = new Graphics();
    bg.beginFill(0x082742);
    bg.drawRect(0, 0, 1, 1);
    bg.endFill();
    bg.scale.set(100);
